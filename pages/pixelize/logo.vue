<template>
    ...
    >
        <div class="w-1/4" id="imgDiv">
            <!-- <img :src="logoToGuess.team_photo" /> -->
        </div>
        <div class="w-1/3">
            <div
                class="flex flex-col-reverse w-10/12 gap-y-2 gap-x-2 items-center justify-center md:flex-row md:w-full"
            >
                <BAutocomplete
                    class="w-full"
                    v-model="search"
...
async function pixelize(attempts: ComputedRef<number>) {
    ...
    let img1 = new Image();
    img1.src = logoToGuess.value?.team_photo;
